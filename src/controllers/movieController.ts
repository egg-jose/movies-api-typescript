...
class movieController {
  ...
  public async getAllMovies(req: Request, res: Response) {
		...
		try {
			...
			return res.status(httpCodes.SERVER_ERROR).send({error: `Internal server error ${error}`});
		}
  }

  public async addMovie(req: Request, res: Response) {
		const { error } = this.validateData(req.body);
		if (error) return res.status(httpCodes.BAD_REQUEST).send(error.details[0].message);
		try {
